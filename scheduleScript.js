...
$('document').ready(function() {
  ...
  if (
    ...
    sessionStorage.getItem('signedIn') !== userName
  ) {
  ...
  function dayClick(time, day) {
    var selectedTime = parseInt(
      $(this)
        .attr('id')
        .slice(0, 2)
        .replace('-', '')
...
async function LoadUserAppointments(database, userName) {
  ...
  $.each(appointmentsArr, function(index, element) {
    for (i = element['startTime']; i <= element['endTime']; i++) {
      ...
      var child = id + ' > span';
      $(child).html(
        times[element['startTime']].value +
          ' - ' +
          times[element['endTime']].value +
          ' ' +
          element['note']
      );
